...
class PermissionModeMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        ignore_list = ['/xadmin', '/api/v1', '/admin', '/v2', '/api/admin/qform/anonymousforminfo/', '/api/admin/qform/anonymousformtemplate/', '/api/admin/qform/anonymoussavefile/']
        for il in ignore_list:
            if request.path.startswith(il):
                return
        white_list = ['/api/admin/logout/', '/api/admin/login/', '/api/admin/api-auth/login/']
