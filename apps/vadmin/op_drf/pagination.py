...
class Pagination(PageNumberPagination):
    ...
    def get_paginated_response(self, data, search_fields=''):
        return SuccessResponse(OrderedDict([
            ...
            ('results', data),
            ('pageSize', self.request.query_params.get(self.page_size_query_param, self.page_size)),  # 兼容d2-crud-plus
            ("pageNum", self.request.query_params.get(self.page_query_param, 1))  # 兼容d2-crud-plus
        ]))
...
class JsonPagination(Pagination):
    ...
    def get_paginated_response(self, data, search_fields=''):
        return SuccessResponse(OrderedDict([
            ...
            ('previous', self.get_previous_link()),
            ('results', data),
            ('pageSize', self.page_size),  # 兼容d2-crud-plus
            ("pageNum", self.request.query_params.get(self.page_query_param, 1))  # 兼容d2-crud-plus
        ]))
