...
from apps.vadmin.system.filters import DictDetailsFilter, DictDataFilter, ConfigSettingsFilter, MessagePushFilter, \
    SaveFileFilter, LoginInforFilter, OperationLogFilter, CeleryLogFilter, SaveFileListFilter
from apps.vadmin.system.models import DictData, DictDetails, ConfigSettings, SaveFile, MessagePush, SaveFileRelation
...
from apps.vadmin.system.serializers import DictDataSerializer, DictDataCreateUpdateSerializer, DictDetailsSerializer, \
    ...
    ExportCeleryLogSerializer, SaveFileListSerializer
...
from common.tasks import clear_all_files
...
class SaveFileModelViewSet(CustomModelViewSet):
   ...
   """
    ...
    def perform_destroy(self, instance):
        # 字典处理
        for i in instance:
            SaveFileRelation.objects.filter(file_id=i.id).delete()
        instance.delete()
    ...
    def clearsavefile(self, request: Request, *args, **kwargs):
        ...
        n = clear_all_files(os.path.join(settings.MEDIA_ROOT, 'system'))
        return SuccessResponse(msg=f"成功清理废弃文件{n}个")
    ...
    def download_file(self, request: Request, *args, **kwargs):
        ...
        file = SaveFile.objects.filter(id=id)
        if len(file) != 0:
            return download_response(os.path.join(settings.MEDIA_ROOT, str(file[0].file)))
        else:
            raise Http404


class SaveFileListViewSet(mixins.ListModelMixin, GenericViewSet):
    """
   文件管理 模型的CRUD视图
   """
    queryset = SaveFile.objects.all()
    serializer_class = SaveFileListSerializer
    filter_class = SaveFileListFilter
    extra_filter_backends = [DataLevelPermissionsFilter]
