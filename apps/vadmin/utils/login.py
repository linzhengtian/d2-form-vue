...
class VJSONWebTokenSerializer(JSONWebTokenSerializer):
    def validate_password(self, value):
        if settings.CRYPT_CHECK:
            crypt = CryptPwd()
            pid = self.initial_data.get("publicId")
            value = crypt.de_js_encrypt_cache(value, pid)
        return value
...
class LoginView(ObtainJSONWebToken):
    ...
    serializer_class = VJSONWebTokenSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        pid = request.data.get('publicId', None)
        if not pid and settings.CRYPT_CHECK:
            return ErrorResponse(msg='密钥失效，请刷新页面')
        ...
        if serializer.is_valid():
            ...
            if self.JWT_AUTH_COOKIE and token:
                ...
                response.set_cookie(self.JWT_AUTH_COOKIE,
                                    ...
                                    httponly=False)
            if pid and settings.CRYPT_CHECK:
                cache.delete("login_private_" + pid)
            return response
