<template>
  <div class="app-container">
      ...
      <el-table-column label="文件本地地址" align="center" prop="file" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <el-button
            ...
            @click="CopyFileUrl(scope.row.file_url ? baseUrl + scope.row.file_url : scope.row.file)"
          />
          &nbsp;
          <span>{{ scope.row.file_url ? baseUrl + scope.row.file_url : scope.row.file }}</span>
        </template>
      </el-table-column>
    ...
    />
    <!-- 对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="400px" append-to-body @close="closeForm"> <!--父组件open变更后，会触发close方法，子组件关闭时间用:before-close-->
      <FileUpload ref="saveFile" :file-type="['ALL']" :autoUpload="false" :multiple="true" :limit="3" :fileSize="10" @closeUpload="closeUpload" />
...
export default {
  ...
  methods: {
    ...
    /** 完成按钮 */
    submitForm() {
      this.$refs.saveFile.submit();
    },
    /** 完成上传 */
    closeUpload([status, fileLists]) {
      if (status) {
        this.open = false;
      }
    },
