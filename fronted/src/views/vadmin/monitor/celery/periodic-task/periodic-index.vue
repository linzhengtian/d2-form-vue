...
export default {
  ...
  methods: {
    ...
    async handleOpenEditPeriodicForm(create = false, info) {
      const resp1 = await SyncDataApi.listCrontabSchedule({ pageSize: 1000 });
      this.$store.state.Crontab = resp1.data.results || [];
      const resp2 = await SyncDataApi.listIntervalschedule({ pageSize: 1000 });
      this.$store.state.Interval = resp2.data.results || [];
      if (create) {
