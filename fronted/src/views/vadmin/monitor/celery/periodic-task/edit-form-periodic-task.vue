  ...
  >
    <el-form ref="form" v-loading="loading" :model="form" :size="$ELEMENT.size" label-width="120px">
      ...
      <el-form-item :rules="[{ required: true, message: '名称不能为空'}]" prop="name" label="名称:">
        <el-input v-model="form.name" placeholder="例如: XXX同步任务" style="width: 80%;"/>
      </el-form-item>
      <el-form-item prop="interval" label="任务频率:">
        <el-select v-model="form.interval" placeholder="请选择任务频率" style="width: 80%;" @change="form.crontab = ''">
          <el-option
      ...
      <el-form-item prop="crontab" label="任务定时:">
        <el-select v-model="form.crontab" placeholder="请选择任务定时" style="width: 80%;" @change="form.interval = ''">
          <el-option
