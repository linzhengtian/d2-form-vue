...
// 通用下载方法
export function download(file_url, fileName, parse) {
  getBlob(parse ? file_url : process.env.VUE_APP_BASE_API + file_url, function(blob) {
    saveAs(blob, fileName);
  });
}

export function downloadAuth(url, fileName) {
  axios({
    method: "get",
...
export function downloadBlob(res, fileName = "结果.xlsx", type = "application/octet-stream; charset=UTF-8") {
  // 调用接口获取到文件，适配后端download_response方法
