...
class SFTP:
    ...
    def upload_file(self, upload_file, upload_file_path, upload_path=None, override=False):
        ...
        try:
            if not upload_path:
                if self.username == 'root':
                    ...
                    upload_path = '/home/{}'.format(self.username)
            self.sftp.stat(upload_path)
            if not override and self.sftp.stat(os.path.join(upload_path, upload_file.name)):
                return
            self.put_file(local_file, upload_path)
        except Exception as e:
            logging.getLogger().error(e)
            return e
