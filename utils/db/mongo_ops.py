    ...
    def __client(self):
        if self.username and self.password:
            ...
            uri = "mongodb://{}:{}@{}:{}/?authSource={}".format(self.username, self.password, self.host, self.port,
                                                                self.authSource)
            # uri = "mongodb://{}:{}@{}:{}/?authSource={}&authMechanism=SCRAM-SHA-1".format(self.username, self.password, self.host, self.port,
            #                                                     self.authSource) # MongoDB 3.0（对应pymongo2.8）之后默认使用“SCRAM-SHA-1”加解密，之前使用的是“MONGODB-CR”。
