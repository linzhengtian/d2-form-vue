...
import cx_Oracle
import logging
from cx_Oracle import OperationalError, ProgrammingError
...
class OraclePool:
    ...
    def __init__(self, host, port, user, password, db, max_pool_size=10, charset='utf8', timeout=5, dsn=None):
        self.host = host
        self.port = port
        self.user = user
    ...
    def _initialize_pool(self):
        ...
        dsn = cx_Oracle.makedsn(self.host, self.port, service_name=self.db) if not self.dsn else self.dsn
        for i in range(self.max_pool_size):
            conn = Connection(user=self.user, password=self.password, dsn=dsn,
                              encoding=self.charset)
            # conn.callTimeout=self.timeout
            self.pool.put_nowait(conn)

    def exec_sql_one(self, sql, **kwargs):
        """
    ...
    def exec_select(self, sql, size=None, one=False, **kwargs):
        """
    ...
    def exec_sql_many(self, sql, *args):
        """
    ...
    def get_tables(self, sql='select table_name from user_tables'):
        """
        获取当前数据库所有的表
        """
    ...
    def get_dbs(self, sql='show databases'):
        ...
        names = [d[0] for d in dbs if d[0] not in ('information_schema', 'performance_schema', 'oracle')]
        return names

    def get_all_users(self):
        users = self.exec_select("SELECT username FROM all_users")
        return (user for user in users[1])
...
if __name__ == '__main__':
    m = OraclePool('10.1.7.198', 3306, 'cc', '123456', 'mysql')
