...
# -*- coding=utf-8 -*-
import redis
from conf.logger import configuration_logger


class RedisOps:
    def __init__(self, host, port, db, password=None, logger=configuration_logger):
    ...
    def lpush(self, rediskey, *values):
        ...
        try:
            ...
            self.logger.error("添加数据失败：{}".format(e))
    ...
    def lpop(self, rediskey):
        ...
        try:
            ...
            self.logger.error("获取数据（lpop）失败：{}".format(e))
    ...
    def rpop(self, rediskey):
        ...
        try:
            ...
            self.logger.error("获取数据（rpop）失败：{}".format(e))
    ...
    def lrange(self, rediskey):
        ...
        try:
            ...
            self.logger.error("获取数据（lrange）失败：{}".format(e))

    def delete(self, *args):
        try:
            self.redis_conn.delete(*args)
        except Exception as e:
            self.logger.error("删除数据失败：{}".format(e))

    def set(self, rediskey, value):
        try:
            self.redis_conn.set(rediskey, value)
        except Exception as e:
            self.logger.error("设置数据失败：{}".format(e))
    ...
    def get(self, rediskey):
        try:
            ...
            self.logger.error("获取数据（get）失败：{}".format(e))
    ...
    def mget(self, *args):
        try:
            ...
            self.logger.error("获取多个数据（mget）失败：{}".format(e))

    def sadd(self, rediskey, *values):
        try:
            self.redis_conn.sadd(rediskey, *values)
        except Exception as e:
            self.logger.error("设置数据集合失败：{}".format(e))
    ...
    def smembers(self, rediskey):
        try:
            ...
            self.logger.error("获取数据集合失败：{}".format(e))
